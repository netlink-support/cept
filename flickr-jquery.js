    ...
    function Plugin(element, options)
    {
        ...
        this._printError = function () {
            this.element.find('.gallery-container').append(
                $("<div></div>", { "class": "col-lg-12 col-lg-offset-1" })
                .append(
                    $("<div></div>", { "class": "error-wrapper" })
                    .append(
                        $("<span></span>", { "class": "label label-danger error" })
        ...
        this._flickrAnimate = function () {
            this.element.find('.gallery-container img').each(
                $.proxy(
                    function (index, el) {
                        var image = el;
                        setTimeout(
            ...
            $.each(
                photos, function (key, photo) {
                    ...
                    element.append(
                        ...
                        .append(
                            $('<a></a>', { 'class': '', href: photo.href, 'data-gallery': '',title: title }).hide()
                            .append(img)
                        )
                    );
                }
        ...
        this._flickrPhotoset = function (photoset) {
            ...
            photos[photoset.id] = [];
            $.each(
                photoset.photo, function (key, photo) {
